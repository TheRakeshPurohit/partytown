...
| `resolveSendBeaconRequestParameters` | Hook that is called before every `navigator.sendBeacon` request to override its fetch parameters, e.g. `keepalive`. The hook uses the API: `resolveSendBeaconRequestParameters(url: URL, location: URL)` and returns a `SendBeaconParameters` object, or `null`/`undefined` to keep the defaults. |
| `mainWindowAccessors`     | An array of global names the worker reads from the main thread window instead of its own, e.g. `mainWindowAccessors: ["mySettings"]`. Useful when a script running in the worker needs data or methods from an object created on the main thread. Note that [forwarded](/forwarding-events) globals stay local to the worker.  |
...
| `fallbackTimeout`         | A timeout in ms until Partytown initialization is considered as failed & fallbacks to the regular execution in main thread. Set to `0` to disable the fallback entirely. Default is 9999                                                                                    |
